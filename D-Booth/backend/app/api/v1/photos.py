...
async def get_photos(
    ...
    if event_id:
        ...
        if not event:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="Event not found for session"
            )
        await check_team_member(event.team_id, current_user, db)
    else:
...
async def upload_photo(
    ...
    bp = BeautyParams(
        smooth=beauty_smooth, whiten=beauty_whiten,
        thinFace=beauty_thinFace, bigEye=beauty_bigEye,
        eyeLight=beauty_eyeLight, acne=beauty_acne,
