...
from app.services.gopro_service import gopro_controller, GoProDevice, GoProStatus
...
async def get_gopro_status(current_user: User = Depends(get_current_active_user)):
    ...
    return {
        ...
        } if gopro_controller.device else None,
        "status": {
...
async def take_gopro_photo(current_user: User = Depends(get_current_active_user)):
    ...
    return {
        "success": True,
        "temp_url": f"{base_url}/temp/{filename}",
