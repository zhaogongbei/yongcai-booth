...
class AITaskRepository(BaseRepository[AITask]):
    ...
    async def get_by_team(self, team_id: UUID, skip: int = 0, limit: int = 100) -> List[AITask]:
    ...
    async def get_by_workflow(
        self, team_id: UUID, workflow: str, skip: int = 0, limit: int = 100
    ) -> List[AITask]:
        """Get tasks by workflow"""
        result = await self.db.execute(
            select(AITask)
            .where(AITask.team_id == team_id, AITask.workflow == workflow)
            .order_by(AITask.created_at.desc())
            .offset(skip)
            .limit(limit)
        )
    ...
    async def get_by_status(
        self, team_id: UUID, status: str, skip: int = 0, limit: int = 100
    ) -> List[AITask]:
        """Get tasks by status"""
        result = await self.db.execute(
            select(AITask)
            .where(AITask.team_id == team_id, AITask.status == status)
            .order_by(AITask.created_at.desc())
            .offset(skip)
            .limit(limit)
        )
    ...
    async def get_total_cost(self, team_id: UUID) -> Decimal:
        ...
        result = await self.db.execute(
            select(func.sum(AITask.actual_cost)).where(AITask.team_id == team_id)
        )
        return result.scalar_one() or Decimal(0)

    async def count_by_team_between(
        self, team_id: UUID, start_at: datetime, end_at: datetime
    ) -> int:
