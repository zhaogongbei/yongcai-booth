...
def upgrade() -> None:
    ...
    if dialect == "postgresql":
        op.execute("CREATE TYPE propcategory AS ENUM ('节日', '婚礼', '生日', '动物', '眼镜', '帽子', '胡须', '自定义')")
        op.execute("CREATE TYPE boothstatus AS ENUM ('online', 'offline', 'busy', 'error')")
        op.execute("CREATE TYPE triggertype AS ENUM ('session_start', 'countdown_start', 'capture_start', 'file_download', 'processing_start', 'sharing_screen', 'session_end', 'printing')")
    ...
    op.create_table('survey_responses',
        ...
        sa.Column('updated_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=False),
        sa.ForeignKeyConstraint(['event_id'], ['events.id'], ),
        sa.ForeignKeyConstraint(['session_id'], ['photo_sessions.id'], ),
        sa.PrimaryKeyConstraint('id')
    )
