...
### 修复
- 修复前端 Green Screen 设置未持久化却提示保存成功的问题，改为显示保存暂不可用。
- 修复 Props API 被重复挂载为 `/api/v1/props/props` 的路径问题、`X-Team-Id` 团队解析 500 问题，并让上传/删除业务规则错误返回 400。
- 修复 Green Screen 设置更新接口未持久化却返回更新成功的问题，改为明确返回 501。
- 修复打印机校准接口未持久化却返回保存成功的问题，改为明确返回 501。
- 修复团队唯一 owner 可移除或降级自己导致团队无 owner 的数据完整性问题，并让成员管理业务错误返回正确 HTTP 状态。
- 修复打印机取消任务和测试页接口把驱动失败误报为 500 的问题，保留正确的 400 业务失败响应。
