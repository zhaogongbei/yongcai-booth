...
- 当前打印预览已能按前端模板 layout 渲染照片框、文本、形状、图片占位和二维码占位；后端真实打印任务仍只提交 photo_id/printer/copies，尚未把模板合成结果作为出纸文件。
...
- 打印后端仍未把当前模板 layout 与照片合成为最终出纸文件；下一轮应评估 `template_render_service.py`、`PrintJob` schema/迁移和打印任务执行链路，确保预览与真实打印一致。
